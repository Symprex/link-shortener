...
**Impact: MEDIUM** - Treat composables as reusable, stateful building blocks and keep their code organized by feature concern. In Signature365, page-level behaviour usually belongs in page models, while composables cover reusable browser logic, smaller cross-component behaviour, and non-page concerns. This keeps large components maintainable and prevents hard-to-debug mutation and API design issues.

## Composable boundary

The repo's own rules on composables and frontend architecture win over anything here.

- Page-level route behaviour should usually live in `use<PageName>Model(...)`, not in ad hoc page composables.
- Reach for composables when the logic is reusable below the page-model boundary, cross-cutting, or not the canonical contract for a page.
- Prefer existing VueUse helpers and repo wrappers before writing custom low-level composables. Load the `vueuse-functions` skill when choosing a VueUse composable; [`vueuse.md`](vueuse.md) is the thin repo-specific pointer, with concrete repo wrappers in `frontend/apps/portal/src/boot/api.ts` and `frontend/packages/common/src/lib/useDataLossPrevention.ts`.
- If a composable talks to the generated API, inject dependencies and test it with `MockApiRequestHandler` rather than hiding the API behind globals.
