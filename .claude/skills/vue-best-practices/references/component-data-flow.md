...
**Impact: HIGH** - Vue components stay reliable when data flow is explicit: props go down, events go up, `v-model` handles two-way bindings, and provide/inject supports cross-tree dependencies. In Signature365, this sits inside a stronger page contract: loaders feed page models, page containers pass the full model once, and child components consume slices or plain props. Blurring those boundaries leads to stale state, hidden coupling, and hard-to-debug UI.
...
The repo's own frontend architecture rules win over the generic rules below.

- `*Page.vue` route containers own loaders and create the page model.
- The matching presentational page component receives the full page model exactly once.
- Components below that level should receive slices or plain props rather than the full page model.
- Inside those presentational components, keep normal Vue data flow explicit: props down, events up, deliberate `defineModel`, and imperative refs only when unavoidable.

## Task List

- Pass full page models only at the top-level page boundary.
- Prefer slices or plain props below the page boundary.
