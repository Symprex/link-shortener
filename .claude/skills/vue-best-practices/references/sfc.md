...
Use `useTemplateRef()` to access template refs. The repo is already on Vue 3.5+, so treat this as the default approach rather than a conditional upgrade path.
